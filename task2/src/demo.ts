...
function reserve(name: string, persons: number): void {
  ...
  if (reservations.length < 5) {
    ...
    console.log(
      `Reservation for ${newReservation.name} with ${newReservation.persons} persons is confirmed.`
    );
  } else {
    waitingReservation.push(newReservation);
    let wTime: number = waitingTime * Math.ceil(waitingReservation.length / 5);
    console.log(
      `Sorry,${newReservation.name} tables are full.Your approximate waiting time: ${wTime} minutes.`
    );
  }
}
